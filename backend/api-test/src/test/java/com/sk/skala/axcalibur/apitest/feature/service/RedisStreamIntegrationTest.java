...
class RedisStreamIntegrationTest {
    ...
    void redisStreamIntegration_shouldProcessGetRequest() { // Given
        ...
        await()
                ...
                .untilAsserted(() -> {
                    ...
                    assertThat(pendingMessages.getTotalPendingMessages()).isZero();

                    // 메시지가 성공적으로 처리된 후 last-delivered-id 확인
                    var lastDeliveredId = getLastDeliveredId(StreamConstants.STREAM_KEY, StreamConstants.GROUP_NAME);
    ...
    void redisStreamIntegration_shouldProcessPostRequest() {
        ...
        ApiTaskDto apiTaskDto = createApiTaskDto("POST", "https://httpbin.org/post", null, requestBody); // When
        var dataMap = ApiTaskDtoConverter.toMap(apiTaskDto);
        var record = MapRecord.create(StreamConstants.STREAM_KEY, dataMap);
        ...
        await()
                ...
                .untilAsserted(() -> {
                    ...
                    assertThat(lastDeliveredId).isNotNull();
                    assertThat(lastDeliveredId)
                            .as("last-delivered-id should not be initial value after processing")
                            .isNotEqualTo("0-0");
                    assertThat(pendingMessages).isNotNull();
                    assertThat(pendingMessages.getTotalPendingMessages()).isZero();
                });
    ...
    void redisStreamIntegration_shouldProcessRequestWithHeaders() {
        ...
        ApiTaskDto apiTaskDto = createApiTaskDto("GET", "https://httpbin.org/headers", headers, null);
    ...
    void redisStreamIntegration_shouldRetainMessageOnError() {
        // Given - 접근할 수 없는 URL
        ApiTaskDto apiTaskDto = createApiTaskDto("GET", "http://non-existent-domain-12345.com/test", null, null);
        ...
        await()
                ...
                .untilAsserted(() -> {
                    ...
                    assertThat(lastDeliveredId).isNotNull();
                    assertThat(lastDeliveredId)
                            .as("last-delivered-id should be initial value")
                            .isNotEqualTo("0-0");
    ...
    private String getLastDeliveredId(String streamKey, String groupName) {
        var groupsInfo = redisTemplate.opsForStream()
                .groups(streamKey);

        return groupsInfo.stream()
