...
public class TcPayloadServiceImpl implements TcPayloadService{
    ...
    private DbDesignRepository dbDesignRepository;
    ...
    public TcRequestPayload buildPayload(ScenarioEntity scenario, List<DbTableDto> dbList) {
        // 1. API매핑표 관련 정보 조회
        List<MappingEntity> mappings = mappingRepository.findByScenario_Id(scenario.getId());
        ...
        List<ApiMappingDto> apiMappingList = apis.stream()
            .map(api -> {   // apis List 순회하며 각 api 별 DTO 생성
                List<ParameterEntity> params = parameterRepository.findByApi_Id(api.getKey()); // 해당 API의 모든 파라미터 가져오는 작업

                List<ApiParamDto> paramDtoList = params.stream()
                    .map(param -> ApiParamDto.builder() // 조회된 파라미터 리스트를 DTO로 변환
                ...
                return ApiMappingDto.builder()
                    .step(mappings.stream() // API와 연결된 MapingEntity 찾아 그에 해당하는 step값 추출
