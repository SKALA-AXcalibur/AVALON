...
def create_batch_mapping_prompt(scenarios_batch: List[Dict], api_lists: List[Dict], batch_num: int) -> str:
    """
    배치 처리용 매핑표 생성 프롬프트 - 토큰 한계 해결
    """
    scenarios_text = "\n".join([
        f"{s.get('scenarioId')}: {s.get('title')} - {s.get('validation', '')[:50]}..."
        for s in scenarios_batch
    ])
    
    apis_text = "\n".join([
        f"API-{i+1}: {api.get('apiName')} - {api.get('description', '')[:50]}..."
        for i, api in enumerate(api_lists)
    ])
    
    return f"""
배치 {batch_num}: 다음 시나리오들에 대한 API 매핑표를 생성하세요.
