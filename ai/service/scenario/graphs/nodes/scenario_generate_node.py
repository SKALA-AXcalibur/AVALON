...
def scenario_generate_node(state: ScenarioState) -> Dict[str, Any]:
    ...
    try:
        ...
        # 피드백이 있으면 포함해서 생성
        if feedback_data:
            scenario_response = generator.generate_scenario_with_feedback(request_data, feedback_data)
        else:
            scenario_response = generator.generate_scenario_request(request_data)
        
        logging.info(f"시나리오 생성 완료: {len(scenario_response.scenario_list)}개 시나리오")
