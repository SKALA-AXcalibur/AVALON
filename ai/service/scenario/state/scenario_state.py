...
def update_generation_success(
    state: ScenarioState, scenarios: ScenarioResponse
) -> Dict[str, Any]:
    """생성 성공시 상태 업데이트"""
    return {
...
def update_validation_success(
    state: ScenarioState, validation: ScenarioValidationResponse
) -> Dict[str, Any]:
    """검증 성공시 상태 업데이트"""
    return {
...
def update_feedback_prepared(
    state: ScenarioState, feedback: FeedbackData
) -> Dict[str, Any]:
    """피드백 준비 완료시 상태 업데이트"""
    return {
...
def is_generation_completed(state: ScenarioState) -> bool:
    ...
    return (
        ...
        and state.get("generated_scenarios") is not None
    )
...
def is_validation_completed(state: ScenarioState) -> bool:
    """검증이 완료되었는지 확인"""
    return (
        state.get("validation_status") in ["pass", "review_required", "failed"]
...
def should_continue_workflow(state: ScenarioState) -> bool:
    ...
    return (
        ...
        and state.get("overall_score", 0) < 80
    )
