...
- **예외 상황 포함**: 파일 업로드 실패, API 호출 오류, 네트워크 장애 등 실제 발생 가능한 예외 케이스 포함
- **완전한 독립성**: 각 시나리오는 다른 시나리오의 성공/실패와 무관하게 독립적으로 실행되도록 설계
- **구체적 검증 기준**: "성공 확인" 대신 "응답 코드 200, 생성된 ID 반환, 3초 이내 응답" 같은 측정 가능한 구체적 기준 명시
...
{{
  "scenario_list": [
    {{
      ...
      "title": "게시글 작성 플로우",
      "description": "사용자 로그인부터 게시글 작성, 로그아웃까지의 전체 사용자 여정을 통해 인증 토큰 관리와 컨텐츠 생성 기능의 완전성을 검증하는 핵심 비즈니스 플로우",
      "validation": "로그인 시 JWT 토큰 생성 확인, 게시글 작성 API 성공 및 posts 테이블 저장, user_id 매핑 검증, 로그아웃 시 토큰 무효화 확인",
      "api_list": [
...
## 시나리오 생성 규칙:
1. **시나리오 제목**: **공백 포함 10자 이내**로 핵심 기능을 압축하여 표현 (예: "로그인 플로우", "결제 처리", "파일 업로드")
2. **설명**: 전체 플로우가 무엇을 검증하는지 구체적으로 기술
3. **검증 포인트**: API 응답 검증, 데이터베이스 테이블 관계, 데이터 정합성 등 실무적 검증 항목 (200자 이내로 간결하게)
...
2. validation 필드는 200자 이내로 간결하게 작성하세요
3. 추가 설명 텍스트 없이 JSON만 출력하세요
4. 제목은 공백 포함 10자 이내로 핵심 내용만 담아 생성하세요
"""
