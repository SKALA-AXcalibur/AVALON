...
class MappingEntity(Base):
    ...
    scenarioKey = Column(Integer, ForeignKey("scenario.key"), nullable=False)  # 시나리오 FK
    apiListKey = Column(Integer, ForeignKey("api_list.key"), nullable=False)  # API 목록 FK

    # 관계 설정
    scenario = relationship("Scenario", back_populates="mappings")
    api_list = relationship("ApiList", back_populates="mappings")

    def __init__(self, id: str, step: int, scenarioKey: int, apiListKey: int):
        self.id = id
        self.step = step
        self.scenarioKey = scenarioKey
        self.apiListKey = apiListKey
