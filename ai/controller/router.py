...
async def generate_scenario(request: ScenarioRequest):
    ...
    try:
        ...
        if generated_scenarios is None:
            error_msg = final_state.get("error_message", "시나리오 생성에 실패했습니다.")
            logging.error(f"시나리오 생성 실패 - 프로젝트: {request.project_id}, 오류: {error_msg}")
            raise HTTPException(status_code=500, detail=f"Failed to generate scenario. {error_msg}")
        
        # 성공 시 생성된 시나리오 개수도 로깅
        scenario_count = len(generated_scenarios.scenario_list) if hasattr(generated_scenarios, 'scenario_list') else 0
        logging.info(f"시나리오 생성 성공 - 프로젝트: {request.project_id}, 생성된 시나리오: {scenario_count}개")
        ...
        logging.exception(f"시나리오 생성 중 예외 발생 - 프로젝트: {request.project_id}")
